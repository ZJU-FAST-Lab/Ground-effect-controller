...
The thrust (**T_i**) and torque (**M_i**) generated by a single rotor [Brandt, 2011; Zhang, 2022] are:
...
The rotor speeds are controlled through the throttle input (**t_i^{des}** ∈ [0,1]), and the rotational speeds are fed back through *BDhot*.  
When the flight controller receives a throttle control signal (**t_c** ∈ [0,1]), the rotor speed will be maintained at a roughly determined value. The relationship between the rotor speed and the throttle, after eliminating the influence of battery voltage, is usually a quadratic function:
...
The rotor speeds require closed-loop control, but we do not implement it for each motor individually.  
Instead, we implement closed-loop control on the combined acceleration generated by all rotors along **${\bm{z}}_B$**.  
The control method for rotor speed is:

\[
t_i^{des} = t_i^{ref} + t_E \\
t_i^{ref} = n_{esc}^{-1}\left( n_i \right) \\
t_E = K_P^T T_a^E + K_I^T \sum T_a^E \\
T_a^E = \left( \sum_{i=1}^4 {k_T} n_i^{des^2} - \sum_{i=1}^4 {k_T} n_i^2 \right)/m,
\]

where:
- **$t_i^{ref}$** is the feedforward throttle obtained by **$n_{esc}^{-1}$**, the inverse function of the earlier equation,
...
The parameters in (\ref{equ_single_rotor_thrust}) are calibrated on the single-rotor platform in Fig.~\ref{fig_model_valid}(a).

Fig.~\ref{sigle_motor_platform} shows the actual and predicted thrust and torque generated by the rotor.



We collect rotor speed and throttle data on the single-motor platform and calibrate the model. The data is illustrated in Fig.~\ref{id_single_motor}.
